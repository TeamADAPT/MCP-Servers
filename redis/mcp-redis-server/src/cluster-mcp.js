...
/**
 * Redis Cluster MCP Server Implementation
 * Direct implementation that follows MCP protocol exactly
 * With support for Redis Cluster mode
 */
...
// Initialize Redis client with support for cluster mode
let redis;
try {
  ...
  if (isClusterMode) {
    ...
    try {
      ...
      if (nodesEnv) {
        ...
        logger.info(`Using cluster nodes from env: ${nodesEnv}`);
      } else {
        // Default to single node with configured host/port
        nodes = [{
          host: process.env.REDIS_HOST || 'localhost',
          port: parseInt(process.env.REDIS_PORT || '7000')
        }];
        logger.info(`Using single node as cluster: ${JSON.stringify(nodes)}`);
      }
      ...
      if (process.env.REDIS_USERNAME && process.env.REDIS_PASSWORD) {
        clusterOptions.redisOptions.username = process.env.REDIS_USERNAME;
        clusterOptions.redisOptions.password = process.env.REDIS_PASSWORD;
      } else if (process.env.REDIS_PASSWORD) {
        clusterOptions.redisOptions.password = process.env.REDIS_PASSWORD;
      }
      
      redis = new Redis.Cluster(nodes, clusterOptions);
      logger.info('Redis Cluster client initialized');
    } catch (e) {
      logger.error('Error parsing cluster nodes', e);
      throw e;
    }
  } else {
    // Standard Redis client for non-cluster mode
    const config = {
      host: process.env.REDIS_HOST || 'localhost',
  ...
  try {
    ...
    if (method === 'call_tool') {
      ...
      if (name === 'list') {
        ...
        // Note: keys command is not recommended in production for large databases
        // In a real implementation, you should use SCAN for better performance
