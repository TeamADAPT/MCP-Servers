import { isGetStreamMessagesArgs } from '../types.js';
...
export async function getStreamMessages(redis, args) {
    if (!isGetStreamMessagesArgs(args)) {
        throw new Error('Invalid arguments: stream parameter is required');
    }
    const rawMessages = await redis.xRead([{ key: args.stream, id: args.start || '0' }], { COUNT: args.count || 1 });
